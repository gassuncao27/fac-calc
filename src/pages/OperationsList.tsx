            ...
            <table className="w-full min-w-[880px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left text-[12px] uppercase tracking-wide text-slate-400">
                  ...
                  {COLUMNS.slice(2).map((col) => (
                    <th key={col.key} className="px-5 py-3.5 text-right font-medium">
                      <button
                        type="button"
                        onClick={() => toggleSort(col.key)}
                        className="inline-flex items-center gap-1 uppercase tracking-wide hover:text-slate-600"
              ...
              <tbody>
                {filtered.map((op) => (
                  ...
                  >
                    <td className="whitespace-nowrap px-5 py-4 font-medium text-slate-900">{op.operationNumber}</td>
                    <td className="tabular whitespace-nowrap px-5 py-4 text-slate-600">{formatDate(op.operationDate)}</td>
                    <td className="max-w-48 truncate px-5 py-4 text-slate-700">{clientName(op.clientId)}</td>
                    <td className="px-5 py-4 text-slate-500">{operationTypeLabel(op.operationType)}</td>
                    <td className="tabular px-5 py-4 text-right text-slate-700">{formatCents(op.nominalAmountCents)}</td>
                    <td className="tabular px-5 py-4 text-right font-medium text-slate-900">{formatCents(op.netAmountCents)}</td>
                    <td className="tabular px-5 py-4 text-right text-slate-500">{formatNumber(op.averageTermDays, 1)}d</td>
                    <td className="tabular px-5 py-4 text-right text-slate-500">{formatPercent(op.monthlyRate)}</td>
